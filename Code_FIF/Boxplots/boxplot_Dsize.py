""" Boxplot on the first dataset w.r.t. the dictionary size.
...
with open("variance_study_score_Dsize_1.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(A)
with open("variance_study_score_Dsize_2.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(B)
with open("variance_study_score_Dsize_3.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(C)
with open("variance_study_score_Dsize_4.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(D)
