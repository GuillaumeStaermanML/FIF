""" Boxplot on the second dataset w.r.t. the number of trees.
...
with open("variance_study_score_N_SD_1.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(A)
with open("variance_study_score_N_SD_2.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(B)
with open("variance_study_score_N_SD_3.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(C)
with open("variance_study_score_N_SD_4.csv", "w") as f_write:
    writer = csv.writer(f_write)
    writer.writerows(D)
